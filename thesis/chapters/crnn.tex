...
With these two observations combined, we conclude that further work on the model to improve the smoothness would might performance a little, but not significantly. However, the model is clearly over-predicting transition, and when being used by a musician, smoothed predictions are valuable to make the chords more interpretable. This motivates the exploration of a decoding step in Section~\ref{sec:decoding}.
