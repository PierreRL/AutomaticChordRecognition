...
\begin{table}[ht]
    ...
    \begin{tabular}{lcccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Results for the HMM and CRF smoothing methods with the \emph{CRNN}. The HMM has almost identical performance to the model with no smoothing. Smoothing must be correcting predictions for roughly the same number of time as it introduces errors on. However, it drastically reduces the number of transition per song to an acceptable level. The CRF performs notably worse than the HMM. }\label{tab:smoothers}
\end{table}
...
While performance does not meaningfully improve on \emph{pop}, the lack of overfitting and the gain on the synthetic data provides hope that with further work and improved generative models, synthetic data could prove a useful tool for ACR. Furthermore, producing hand-annotated data is error-prone and subject to human interpretation. Synthetic data may produce non-identifiable data points but provides an opportunity for consistency and error-free data. Poor performance on synthetic data can only be explained by failures of the model or indeterminacy of the generated samples, not incorrect labels. Regardless, given the lack of performance increase, I do not continue to train on synthetic data.
